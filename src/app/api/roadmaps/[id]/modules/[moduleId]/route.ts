...
export const PUT = async (request: NextRequest, { params }: { params: Promise<{ id: string; moduleId: string }> }) =>
  apiHandler(async () => {
    const resolvedParams = await params;
    const user = await getAuthenticatedUser();
    const body = await request.json();
    const payload = parseModulePayload(body);
    const updatedModule = await updateModule(user.id, resolvedParams.id, resolvedParams.moduleId, payload);
...
export const DELETE = async (_: NextRequest, { params }: { params: Promise<{ id: string; moduleId: string }> }) =>
  apiHandler(async () => {
    const resolvedParams = await params;
    const user = await getAuthenticatedUser();
